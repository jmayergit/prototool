...
```bash
curl -sSL \
  https://github.com/jmayergit/prototool/releases/download/v1.8.0/prototool-$(uname -s)-$(uname -m).tar.gz | \
...
```bash
curl -sSL \
  https://github.com/jmayergit/prototool/releases/download/v1.8.0/prototool-$(uname -s)-$(uname -m) \
...
```bash
GO111MODULE=on go get github.com/jmayergit/prototool/cmd/prototool@dev
```

To install to a specific location, use the `GOBIN` environment variable.

```bash
GO111MODULE=on GOBIN=/path/to/bin go get github.com/jmayergit/prototool/cmd/prototool@dev
```
...
$(PROTOTOOL):
	...
	cd $(PROTOTOOL_TMP); go get github.com/jmayergit/prototool/cmd/prototool@$(PROTOTOOL_VERSION)
	@rm -rf $(PROTOTOOL_TMP)
	@rm -rf $(dir $(PROTOTOOL))
